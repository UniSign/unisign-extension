...
describe('bs58', () => {
  describe('encode', () => {
    test('should encode all valid sample correctly', () => {
      for (const item of validSamples) {
        const result = bs58.encode(Buffer.from(item.payload, 'hex'))
        expect(result).toBe(item.string)
      }
    })
  })

  describe('decode', () => {
    test('should decode all valid sample correctly', () => {
      for (const item of validSamples) {
        const result = bs58.decode(item.string)
        expect(isNil(result)).toBeFalsy()
        expect(result.toString('hex')).toBe(item.payload)
      }
    })

    test('should throw when decoding invalid samples', () => {
      for (const item of invalidSamples) {
        expect(() => bs58.decode(item.string)).toThrow()
      }
    })
  })

  describe('decodeUnsafe', () => {
    test('should decodeUnsafe all valid sample correctly', () => {
      for (const item of validSamples) {
        const result = bs58.decodeUnsafe(item.string)
        expect(isNil(result)).toBeFalsy()
        expect(result.toString('hex')).toBe(item.payload)
      }
    })

    test('should return null when decoding invalid samples', () => {
      for (const item of invalidSamples) {
        const result = bs58.decodeUnsafe(item.string)
        expect(isNil(result)).toBeTruthy()
      }
    })
  })
})
