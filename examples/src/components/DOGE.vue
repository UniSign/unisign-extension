...
import { core, doge } from 'sign.mjs'
  ...
  setup() {
    ...
    return {
      ...
      onSetPrivateKey () {
        ...
        rawKeypair = doge.Keypair.fromHex(privateKey.value)
        publicKey.value = core.util.bufferToHex(rawKeypair.publicKey)

        rawAddress = doge.Address.fromBuffer(rawKeypair.publicKey)
        addressLegacy.value = rawAddress.toLegacyAddress(chainType.value)

        signProvider = doge.SignProvider.create({
          keypairs: [rawKeypair]
        })
      },
      onChangeChainType () {
      ...
      async onSignPlainText () {
        ...
        // const signature = await signProvider.signPlainMessage(keypair, plainText.value, doge.SegwitType.Native)
        // const signature = await signProvider.signPlainMessage(keypair, plainText.value, doge.SegwitType.P2SH)
      ...
      async onSignStructMessage (value) {
        ...
        // Sign message with different address type
        // const signature = await signProvider.signStructMessage(keypair, plainText.value, doge.SegwitType.Native)
        // const signature = await signProvider.signStructMessage(keypair, plainText.value, doge.SegwitType.P2SH)
      ...
      async onSignTransaction () {
        ...
        signProvider.setKeypairs({ keypairs: [
            doge.Keypair.fromHex('7149d6db2352c4d62e0f608aac8e2e2990f26225baaaa875da14f6f3ae7eb6ef'),
            doge.Keypair.fromHex('a950ec757af14446ffa0840a4c92d82c09aab749d835ce9c3e192b1b59174dcd'),
          ]})
